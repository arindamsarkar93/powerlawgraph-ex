...
def run_inference(tr_split=0.8):
	...
	#Inference
	fit = m.sampling(data = data, chains = 4, verbose = False);
	alpha = np.mean(fit.extract('alpha')['alpha']);
	tr_ll = np.mean(fit.extract('tr_log_lik')['tr_log_lik']);
	ts_ll = np.mean(fit.extract('ts_log_lik')['ts_log_lik']);
